...
class IntegrationLogic(BaseIntegrationTemplate):
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "list-profiles":
                resp = await client.get("/v2/profiles")
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-transfers":
                params = {k: v for k, v in payload.items() if k in ("profile", "limit", "offset", "status")}
                resp = await client.get("/v1/transfers", params=params)
                resp.raise_for_status()
                return resp.json()
            ...
            if action_name == "list-accounts":
                ...
                resp = await client.get(f"/v4/profiles/{profile_id}/balances", params=params)
                resp.raise_for_status()
                return resp.json()
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "wise"}
