"""Auto-generated integration for Wachete."""
...
class IntegrationLogic(BaseIntegrationTemplate):
    """Integration for Wachete (api_key)."""

    BASE_URL = "https://api.wachete.com"
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "send-sms":
                ...
                r = await client.post("/sms/send", json=body)
                r.raise_for_status()
                return r.json()
            elif action_name == "get-sms-status":
                message_id = payload.get("message_id")
                if not message_id:
                    raise ValueError("'message_id' required")
                r = await client.get(f"/sms/{message_id}/status")
                r.raise_for_status()
                return r.json()
            elif action_name == "get-delivery-report":
                message_id = payload.get("message_id")
                if not message_id:
                    raise ValueError("'message_id' required")
                r = await client.get(f"/sms/{message_id}/delivery")
                r.raise_for_status()
                return r.json()
                ...
                r.raise_for_status()
                return r.json()
            elif action_name == "list-sent-sms":
                params = {k: v for k, v in payload.items() if k not in [] and v is not None}
                r = await client.get("/sms/sent", params=params)
                r.raise_for_status()
                return r.json()
            raise ValueError(f"Unknown action: {action_name}")

    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "wachete"}
