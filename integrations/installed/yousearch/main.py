"""Auto-generated integration for YouSearch."""
...
class IntegrationLogic(BaseIntegrationTemplate):
    """Integration for YouSearch (api_key)."""

    BASE_URL = "https://api.yousearch.com"
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "search-documents":
                ...
                r.raise_for_status()
                return r.json()
            elif action_name == "create-index":
                body = {k: v for k, v in payload.items() if k not in [] and v is not None}
                r = await client.post("/indexes", json=body)
                r.raise_for_status()
                return r.json()
            elif action_name == "get-index":
                index_id = payload.get("index_id")
                if not index_id:
                    raise ValueError("'index_id' required")
                r = await client.get(f"/indexes/{index_id}")
                r.raise_for_status()
                return r.json()
            elif action_name == "delete-index":
                index_id = payload.get("index_id")
                if not index_id:
                    raise ValueError("'index_id' required")
                r = await client.delete(f"/indexes/{index_id}")
                r.raise_for_status()
                return r.json()
            elif action_name == "index-document":
                index_id = payload.get("index_id")
                if not index_id:
                    raise ValueError("'index_id' required")
                body = {k: v for k, v in payload.items() if k not in ['index_id'] and v is not None}
                r = await client.post(f"/indexes/{index_id}/documents", json=body)
                r.raise_for_status()
                return r.json()
            raise ValueError(f"Unknown action: {action_name}")

    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "yousearch"}
