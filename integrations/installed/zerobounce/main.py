"""Auto-generated integration for ZeroBounce."""
...
class IntegrationLogic(BaseIntegrationTemplate):
    """Integration for ZeroBounce (api_key)."""
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "validate-email":
                body = {k: v for k, v in payload.items() if k not in [] and v is not None}
                r = await client.post("/v2/validate", json=body)
                r.raise_for_status()
                return r.json()
            elif action_name == "validate-email-batch":
                body = {k: v for k, v in payload.items() if k not in [] and v is not None}
                r = await client.post("/v2/validate/batch", json=body)
                r.raise_for_status()
                return r.json()
            elif action_name == "get-email-info":
                email = payload.get("email")
                if not email:
                    raise ValueError("'email' required")
                r = await client.get(f"/v2/info/{email}")
                r.raise_for_status()
                return r.json()
            elif action_name == "check-domain":
                domain = payload.get("domain")
                if not domain:
                    raise ValueError("'domain' required")
                r = await client.get(f"/v2/domain/{domain}")
                r.raise_for_status()
                return r.json()
            elif action_name == "get-account-status":
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "zerobounce"}
