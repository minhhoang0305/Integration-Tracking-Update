...
class IntegrationLogic(BaseIntegrationTemplate):
    BASE_URL = "https://api.yonyoucloud.com"
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "list-vouchers":
                params = {k: v for k, v in payload.items() if k in ("pageSize", "pageIndex", "beginDate", "endDate")}
                resp = await client.get("/iuap/api/voucher/list", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-customers":
                params = {k: v for k, v in payload.items() if k in ("pageSize", "pageIndex")}
                resp = await client.get("/iuap/api/customer/list", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-products":
                params = {k: v for k, v in payload.items() if k in ("pageSize", "pageIndex")}
                resp = await client.get("/iuap/api/inventory/list", params=params)
                resp.raise_for_status()
                return resp.json()
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "yonyou"}
