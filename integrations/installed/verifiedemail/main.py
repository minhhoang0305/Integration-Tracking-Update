"""Auto-generated integration for VerifiedEmail."""
...
class IntegrationLogic(BaseIntegrationTemplate):
    """Integration for VerifiedEmail (api_key)."""

    BASE_URL = "https://api.verifiedemail.com/v1"
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "verify-email":
                body = {k: v for k, v in payload.items() if k not in [] and v is not None}
                r = await client.post("/verifications", json=body)
                r.raise_for_status()
                return r.json()
            elif action_name == "get-verification-status":
                verification_id = payload.get("verification_id")
                if not verification_id:
                    raise ValueError("'verification_id' required")
                r = await client.get(f"/verifications/{verification_id}")
                r.raise_for_status()
                return r.json()
            elif action_name == "list-verifications":
                params = {k: v for k, v in payload.items() if k not in [] and v is not None}
                r = await client.get("/verifications", params=params)
                r.raise_for_status()
                return r.json()
            elif action_name == "delete-verification":
                verification_id = payload.get("verification_id")
                if not verification_id:
                    raise ValueError("'verification_id' required")
                r = await client.delete(f"/verifications/{verification_id}")
                r.raise_for_status()
                return r.json()
            raise ValueError(f"Unknown action: {action_name}")

    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "verifiedemail"}
