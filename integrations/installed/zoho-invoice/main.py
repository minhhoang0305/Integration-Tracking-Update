...
class IntegrationLogic(BaseIntegrationTemplate):
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            ...
            elif action_name == "list-estimates":
                params = {k: v for k, v in payload.items() if v is not None}
                r = await client.get("/estimates", params=params)
                r.raise_for_status()
                return r.json()

            elif action_name == "get-estimate":
                estimate_id = payload.get("estimate_id")
                if not estimate_id:
                    raise ValueError("'estimate_id' required")
                r = await client.get(f"/estimates/{estimate_id}")
                r.raise_for_status()
                return r.json()

            elif action_name == "create-estimate":
                body = {k: v for k, v in payload.items() if v is not None}
                r = await client.post("/estimates", json=body)
                r.raise_for_status()
                return r.json()

            elif action_name == "update-estimate":
                estimate_id = payload.get("estimate_id")
                if not estimate_id:
                    raise ValueError("'estimate_id' required")
                body = {k: v for k, v in payload.items() if k not in ["estimate_id"] and v is not None}
                r = await client.put(f"/estimates/{estimate_id}", json=body)
                r.raise_for_status()
                return r.json()

            elif action_name == "delete-estimate":
                estimate_id = payload.get("estimate_id")
                if not estimate_id:
                    raise ValueError("'estimate_id' required")
                r = await client.delete(f"/estimates/{estimate_id}")
                r.raise_for_status()
                return r.json()
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "zoho-invoice"}
