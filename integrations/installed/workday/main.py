...
class IntegrationLogic(BaseIntegrationTemplate):
    def __init__(self):
        ...
        self.base_url = "https://api.workday.com" # TODO: Verify URL
    ...
    async def execute_sync_task(self, sync_name: str = "default", *args, **kwargs) -> Any:
        async with await self._get_client() as client:
            if False:
                pass
            elif sync_name == "employees":
                response = await client.get("/employees")
                response.raise_for_status()
                return response.json()
            elif sync_name == "groups":
                response = await client.get("/groups")
                response.raise_for_status()
                return response.json()
            elif sync_name == "locations":
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "workday"}
