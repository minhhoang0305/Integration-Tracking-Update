...
class IntegrationLogic(BaseIntegrationTemplate):
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "list-contacts":
                params = {k: v for k, v in payload.items() if k in ("page", "page_size", "type", "search")}
                resp = await client.get("/contacts", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-invoices":
                params = {k: v for k, v in payload.items() if k in ("page", "page_size", "status", "issue_date_after")}
                resp = await client.get("/invoices", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-accounts":
                params = {k: v for k, v in payload.items() if k in ("page", "page_size")}
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "wafeq"}
