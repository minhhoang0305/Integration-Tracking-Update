...
class IntegrationLogic(BaseIntegrationTemplate):
    BASE_URL = "https://api.zucchetti.it/v1"
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "list-customers":
                params = {k: v for k, v in payload.items() if k in ("pagina", "righePerPagina", "ricerca")}
                resp = await client.get("/anagrafiche/clienti", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-invoices":
                params = {k: v for k, v in payload.items() if k in ("pagina", "righePerPagina", "dataInizio", "dataFine")}
                resp = await client.get("/fatture/attive", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-employees":
                params = {k: v for k, v in payload.items() if k in ("pagina", "righePerPagina")}
                resp = await client.get("/hr/dipendenti", params=params)
                resp.raise_for_status()
                return resp.json()
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "zucchetti"}
