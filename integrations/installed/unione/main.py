"""Auto-generated integration for Unione."""
...
class IntegrationLogic(BaseIntegrationTemplate):
    """Integration for Unione (api_key)."""

    BASE_URL = "https://api.unione.io"
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "send-message":
                ...
                r.raise_for_status()
                return r.json()
            elif action_name == "get-message-status":
                message_id = payload.get("message_id")
                if not message_id:
                    raise ValueError("'message_id' required")
                r = await client.get(f"/messages/{message_id}/status")
                r.raise_for_status()
                return r.json()
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "unione"}
