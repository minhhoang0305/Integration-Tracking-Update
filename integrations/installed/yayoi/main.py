...
class IntegrationLogic(BaseIntegrationTemplate):
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "list-invoices":
                params = {k: v for k, v in payload.items() if k in ("limit", "offset", "fiscal_year")}
                resp = await client.get("/invoices", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-journals":
                params = {k: v for k, v in payload.items() if k in ("limit", "offset", "start_date", "end_date")}
                resp = await client.get("/journals", params=params)
                resp.raise_for_status()
                return resp.json()

            if action_name == "list-accounts":
                params = {k: v for k, v in payload.items() if k in ("limit",)}
                resp = await client.get("/accounts", params=params)
                resp.raise_for_status()
                return resp.json()
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "yayoi"}
