"""Auto-generated integration for Userlist."""
...
class IntegrationLogic(BaseIntegrationTemplate):
    """Integration for Userlist (api_key)."""

    BASE_URL = "https://api.userlist.com/v1"
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "get-user":
                ...
                if not id:
                    raise ValueError("'id' required")
                body = {k: v for k, v in payload.items() if k not in ['id'] and v is not None}
                r = await client.patch(f"/users/{id}", json=body)
                r.raise_for_status()
                return r.json()
                ...
                if not id:
                    raise ValueError("'id' required")
                r = await client.delete(f"/users/{id}")
                r.raise_for_status()
                return r.json()
                ...
                r.raise_for_status()
                return r.json()
            elif action_name == "search-users":
                email = payload.get("email")
                if not email:
                    raise ValueError("'email' required")
                r = await client.get(f"/users/search?email={email}")
                r.raise_for_status()
                return r.json()
            raise ValueError(f"Unknown action: {action_name}")

    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "userlist"}
