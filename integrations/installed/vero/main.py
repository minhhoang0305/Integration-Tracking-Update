"""Auto-generated integration for Vero."""
...
class IntegrationLogic(BaseIntegrationTemplate):
    """Integration for Vero (api_key)."""

    BASE_URL = "https://api.verosystems.com"
    ...
    async def execute_sync_task(self, sync_name: str = "default", **kwargs) -> Any:
        async with self._client() as client:
            if sync_name == "subscribers":
                r = await client.get("/")
                r.raise_for_status()
                return r.json()
            elif sync_name == "campaigns":
                r = await client.get("/")
                r.raise_for_status()
                return r.json()
    ...
    async def execute_action(self, action_name: str, payload: Dict[str, Any]) -> Any:
        async with self._client() as client:
            if action_name == "get-subscriber":
                ...
                r.raise_for_status()
                return r.json()
            elif action_name == "create-subscriber":
                body = {k: v for k, v in payload.items() if k not in [] and v is not None}
                r = await client.post("/subscribers", json=body)
                r.raise_for_status()
                return r.json()
            elif action_name == "update-subscriber":
                subscriber_id = payload.get("subscriber_id")
                if not subscriber_id:
                    raise ValueError("'subscriber_id' required")
                body = {k: v for k, v in payload.items() if k not in ['subscriber_id'] and v is not None}
                r = await client.put(f"/subscribers/{subscriber_id}", json=body)
                r.raise_for_status()
                return r.json()
            elif action_name == "delete-subscriber":
                subscriber_id = payload.get("subscriber_id")
                if not subscriber_id:
                    raise ValueError("'subscriber_id' required")
    ...
    async def handle_webhook(self, request: Any) -> Any:
        return {"status": "ok", "provider": "vero"}
